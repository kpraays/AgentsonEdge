...
if __name__ == "__main__":
    ...
    src_direc = r"C:\Users\aayus\Downloads\output_plots\output_readings\granular_helper\task-oriented_increasing_prompt_sizes\output"
    dest_direc = r"C:\Users\aayus\Downloads\output_plots\output_readings\granular_helper\task-oriented_increasing_prompt_sizes"
    for path, dirs, files in os.walk(src_direc):
        data_files = [os.path.join(path, file) for file in files]
        data_files = sorted(data_files)
    count = 0
    with open(os.path.join(dest_direc, "processed_granular_fixed_output.csv"), "w", newline="") as csvfile:
        fieldnames = ["centi-secs", "id", "cpu_per", "cpu_mem", "gpu_mem", "gpu_per",  "generation_throughput", "prompt_eval_throughput", "total-centi-secs"]
        ...
        for i, data_file in enumerate(data_files):    
            ...
            for index, value in enumerate(cpu_percent):
                ...
                if cpu_percent[index]!=-1:
                    writer.writerow({"centi-secs": index,
                                     "total-centi-secs": count,
                                    "id": query_id, 
